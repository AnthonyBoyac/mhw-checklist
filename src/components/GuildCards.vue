<template>
  <div class="container gear-container">
    ...
    progressName="Titles" />
    <table id="guild-card-title-table">
      <thead>
        <tr>
          <th>Unlocked</th>
          <th>Name</th>
...
export default {
  ...
  mounted() {
    if (localStorage.getItem('unlockedGCTitles'))
      this.clearedItems = JSON.parse(localStorage.getItem('unlockedGCTitles'))
  },
  watch: {
    // add/remove list item in/from local storage depending whether the checkox is checked or not
    clearedItems: {
      handler() {
        localStorage.setItem('unlockedGCTitles', JSON.stringify(this.clearedItems))
      }
    }
  },
  methods: {
