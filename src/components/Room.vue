<template>
  <div class="container gear-container">
    ...
    progressName="Unlocked decor" />
    <table id="research-table">
      <thead>
        <tr>
          <th>Unlocked Decor</th>
          <th>Decor Name</th>
        </tr>
      </thead>
      <tbody>
...
export default {
  ...
  mounted() {
    if (localStorage.getItem('unlockedRoomDecor'))
      this.clearedItems = JSON.parse(localStorage.getItem('unlockedRoomDecor'))
  },
  watch: {
    // add/remove list item in/from local storage depending whether the checkox is checked or not
    clearedItems: {
      handler() {
        localStorage.setItem('unlockedRoomDecor', JSON.stringify(this.clearedItems))
      }
    }
  },
  methods: {
