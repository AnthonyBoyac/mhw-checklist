<template>
  <div class="container gear-container">
    ...
    progressName="Completed deliveries" />
...
export default {
  ...
  mounted() {
    if (localStorage.getItem('completedDeliveries'))
      this.clearedItems = JSON.parse(localStorage.getItem('completedDeliveries'))
  },
  watch: {
    // add/remove list item in/from local storage depending whether the checkox is checked or not
    clearedItems: {
      handler() {
        localStorage.setItem('completedDeliveries', JSON.stringify(this.clearedItems))
      }
    }
  },
  methods: {
